...
from urllib.parse import urlsplit, SplitResult, parse_qs
...
@app.get('/transcript')
async def get_summary(url: str = Query(default='', description='Youtube URL', example='https://www.youtube.com/watch?v=K5KVEU3aaeQ')):
  ...
  if len(paths) < 1 or paths[0] != 'watch':
    raise BadRequestException(f"Invalid YouTube URL. Path ('/{'/'.join(paths)}\') should be '/watch'")
